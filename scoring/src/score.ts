...
const TEMPLATES: Record<string, string> = {
  added_sugar_high:
    "{added_sugar_g}g added sugar per serving. For cutting, aim for ≤{max_added_sugar}g.",
  protein_low:
    "{protein_g}g protein — below target. Aim for ≥{min_protein}g per serving.",
  calories_high:
    "{calories} calories — calorie-dense for a cutting snack.",
};

function loadGoalConfig(): GoalConfig {
  const path = join(__dirname, "../goals/cutting.json");
  return JSON.parse(readFileSync(path, "utf-8")) as GoalConfig;
}
...
export function scoreProduct(product: Product, config?: GoalConfig): ScoreResult {
  const goalConfig = config ?? loadGoalConfig();
  const category =
  ...
  const addedSugar =
    product.nutrition.added_sugar_g ?? product.nutrition.sugar_g;
  const ppc =
...
export function compareProducts(a: Product, b: Product): CompareResult {
  ...
  const deltas = {
    ...
    added_sugar_g: delta(
      loser.metrics.added_sugar_g,
      winner.metrics.added_sugar_g
    ),
...
export function getTemplate(flagId: string): string | undefined {
  return TEMPLATES[flagId];
}

export { loadGoalConfig };
